...
with open(args.input_file, 'r') as f:
    for line in f.readlines():
        ...
        elif data['type'] in ["boolq", "piqa", "social_i_qa", "hellaswag", "winogrande", "ARC-Challenge", "ARC-Easy", "openbookqa"]:
            y_gt = extract_commonsense_answer(data['type'], data['answer'])
            y_pred = extract_commonsense_answer(data['type'], data['output'])
            if y_pred != None:
                results[data['type']].append(y_pred == y_gt)
            else:
                results[data['type']].append(False)
        else:
            raise ValueError()
...
with open(os.path.join(parent_dir, 'perf.json'), 'w') as f:
    json.dump(perf, f, indent=4)

print(f'results save to: {parent_dir}/perf.json')